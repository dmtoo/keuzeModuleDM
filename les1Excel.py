...
df=pd.read_excel(filenaam)  # linker kant van het = teken  staat waar iets wordt bewaard in dit geval in df
                            # df is een afkorting van DataFrame
print(df.head(8))  # nu krijg je 8 regels te zien , als je niets opgeeft krijg je de eerste 5 regels te zien,  (of een error)
...
df2=df.naam # df.kolomnaam (je hebt een probleem als er een spatie in zit
print(df2.head())
df2=df['naam'] # hier kan je wel rekening houden met spaties , ja mag geen tikfouten maken
print(df2.head(8))
...
# rij selectie
#
print(df[:3])  # geeft de eerste 3 rijen (index 0 t/m 2  [vanaf:tot:step]  step is standaard =1
print(df[1:4])  # geeft vanaf index=1  tot index=4 step=1
print(df[4:1:-1]) # omgekeerd vanaf index=4 tot index=1 -1 geeft aan omgekeerd (zonder -1 werkt mhet niet
#
#  bekijken van de inhoud van df
#
print(df.describe()) #aantal, gemiddelde, standaard deviatie, minimum, Quartile : 25%,50% en 75%, maximum
w=df.describe()
aantal=w['nr'][0]  # nr is kolomnaam , 0 is de index er van
print("aantal=",aantal)

waarde=df[kolomnamen[2]].values
print(waarde)
print("minimum=",min(waarde))
print("maximum=",max(waarde))
waarde[4]=0  # het vervangen dan nan door 0.  MAG NIET in bij DataMining !!!!
...
df3=df[df[kolomnamen[2]]>6.0] # df[kolomnamen[2]]>6.0 lezen als  df['O1]>6.0
print(df3.head())
